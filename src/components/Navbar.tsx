...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ["sobre", "stack", "projetos", "formacao", "certificacoes", "diferenciais", "contato"];
      ...
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between">
          <a href="#" className="relative group">
            <span className="font-display text-xl font-bold text-gradient">Marcelo Batista</span>
            ...
            {navLinks.map((link) =>
              link.isDownload ? (
                <a
                  ...
                  href={link.href}
                  download
                  className="relative px-4 py-2 text-sm font-medium transition-colors duration-300 rounded-lg text-muted-foreground hover:text-foreground"
                >
                  {link.label}
                </a>
              ) : (
          ...
          {navLinks.map((link) =>
            link.isDownload ? (
              <a
                ...
                className="text-lg font-medium py-3 px-4 rounded-lg transition-all duration-300 text-muted-foreground hover:text-foreground hover:bg-card/50"
              >
                {link.label}
              </a>
            ) : (
